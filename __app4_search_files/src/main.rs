...
fn get_search_data() -> Option<(String, String, Vec<String>)> {
    let search_path = match input("Enter path to the dir to search for file in: ") {
        Ok(path) => path,
        Err(_) => return None
    };
    let search_name = match input("Enter filename to search (without extension): ") {
        Ok(path) => path,
        Err(_) => return None
    };
    let search_extensions = match input("Enter file extensions separated by space: ") {
...
fn search_files(search_path: &str, filename: &str,
                extensions: &Vec<String>, now: &Instant, results_count: &mut i32) {
    ...
    for entry in files {
        if let Ok(entry) = entry {
            ...
            } else if is_empty_filename && extensions.contains(&file_extension){  //rebuild in fn filename_matches()
                file_found(&path, now, results_count);
